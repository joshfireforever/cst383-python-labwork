...
# assume 'width' and 'length' are defined
# (write an expression)
width[np.nonzero(np.less(length,5.0))]
...
# (write an expression)
(length - np.min(length))/(np.max(length - np.min(length)) - np.min(length - np.min(length)))
...
# (assignment to parts)
parts = np.reshape(parts, (2,9)).transpose()
...
def distance(width, length, parts):
    # your code here
    return np.array(np.sqrt((width - parts[:,0])**2 + (length - parts[:,1])**2))
...
def most_similar_part(width, length, parts):
    # your code here
    this_dist = np.array(distance(width,length,parts))
    this_min = np.min(this_dist)
    mask = np.less(this_dist-this_min,0.01)
    return np.unique(part_num[mask > 0])
