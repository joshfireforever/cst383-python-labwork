...
num_measles = np.count_nonzero(has_measles == True)
...
tests_positive = np.full(fill_value=False,shape=n)
...
tests_positive[np.where(has_measles)[0][np.random.choice([True,False], size=num_measles, p=[p_pos_if_measles, 1-p_pos_if_measles])]] = True
...
# (update tests_positive)
tests_positive[np.where(has_measles==False)[0][np.random.choice([True,False], size=(n-num_measles), p=[p_pos_if_okay, 1-p_pos_if_okay])]] = True
...
# (expression)
pd.crosstab(has_measles, tests_positive)[1][1]/(pd.crosstab(has_measles, tests_positive)[1][0]+pd.crosstab(has_measles, tests_positive)[1][1])

#@ 7
# Using the has_measles and tests_positive arrays, compute the probability 
# that a person has does not have measles given negative test results.
# (expression)
pd.crosstab(has_measles, tests_positive)[0][0]/(pd.crosstab(has_measles, tests_positive)[0][1]+pd.crosstab(has_measles, tests_positive)[0][0])

#@ 8
# Using the has_measles and tests_positive arrays, compute the probability 
# that a person has measles given negative test results.
# (expression)
pd.crosstab(has_measles, tests_positive)[0][1]/(pd.crosstab(has_measles, tests_positive)[0][1]+pd.crosstab(has_measles, tests_positive)[0][0])
...
# (define a function)
def sim_p_measles_if_pos(per_measles, per_pos_if_measles, per_pos_if_okay):
    measles = np.random.choice([True,False], size=n, p=[per_measles, 1-per_measles])
    count_measles = np.count_nonzero(measles == True)
    positive = np.full(fill_value=False,shape=n)
    positive[np.where(measles)[0][np.random.choice([True,False], size=count_measles, p=[per_pos_if_measles, 1-per_pos_if_measles])]] = True
    positive[np.where(measles==False)[0][np.random.choice([True,False], size=(n-count_measles), p=[per_pos_if_okay, 1-per_pos_if_okay])]] = True
    
    return pd.crosstab(measles, positive)[1][1]/(pd.crosstab(measles, positive)[1][0]+pd.crosstab(measles, positive)[1][1])
...
# test is positive.
# (define a function)
def p_measles_if_pos(per_measles, per_pos_if_measles, per_pos_if_okay):
    return (per_pos_if_measles*per_measles)/((per_pos_if_measles*per_measles)+(per_pos_if_okay*(1-per_measles)))
