...
unemp.sort_values(ascending=True)[0:10]
...
(df.Total*df.ShareWomen).sum()/df.Total.sum()
...
df.sort_values(['Median'],ascending=False)[0:10][['Median','P25th','P75th']]
...
df.sort_values(['Median'],ascending=True)[0:10][['Median','P25th','P75th']]
...
def median_method(df):
    temp = ((df['Median']*df['Major_share']).sum())/(df['Major_share'].sum())
    return temp
df.groupby(['Major_category']).apply(median_method)
...
def ump_method(df):
    temp = ((df['Unemployment_rate']*df['Major_share']).sum())/(df['Major_share'].sum())
    return temp
df.groupby(['Major_category']).apply(ump_method)
...
# Business                               0.487205
(df.groupby(['Major_category']).sum()['Women'])/(df.groupby(['Major_category']).sum()['Total'])
...
def mean_method(df):
    temp = ((df['Median']*df['Major_share']).sum())/(df['Major_share'].sum())
    return temp
df.groupby(['HighShareWomen']).apply(mean_method)
...
# Health                                 0.067504
((df.groupby(['Major_category'])['Low_wage_jobs'].sum())
 /(df.groupby(['Major_category'])['Total'].sum())
...
((df.groupby(['Major_category'])['Unemployed'].sum())
    /((df.groupby(['Major_category'])['Unemployed'].sum())
    +(df.groupby(['Major_category'])['Employed'].sum())
